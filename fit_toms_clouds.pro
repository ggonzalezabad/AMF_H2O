...
For iozo = 0, nozo-1 do begin
   ...
   filename = '../CALCULATIONS/H2O_'+TOMS[iozo]+'_1.00_'+CLDP[ipre]+'_0.80_GC_upwelling_output.nc'
   print, filename
   ; Open and read file
   obj  = Obj_New('NCDF_DATA', filename)
   data = Obj -> ReadFile(filename)
   ...
   PRINT, STRCOMPRESS('OZONE '+TOMS[iozo]+', NWAV '+STRING(NWAV)+', NLAYERS '+STRING(NLAY)+', NLEVELS '+$
          STRING(NLEV)+', CLOUD PRESSURE '+CLDP[ipre])
   ...
   SAVE, nsza, nvza, nraa, nlev, nlay, nwav, zlev, plev,   $
         ...
         I0, I1, I2, Ir, Sb, TOMS_CLI, FILENAME = DIR_FITTING+'/Radiance_fit_'+TOMS[iozo]+'_'+CLDP[ipre]+'.dat'
...
; Saving the results of the Scattering fittings. dI0, dI1, dI2
   SAVE, nsza, nvza, nraa, nlev, nlay, nwav, zlev, plev, $
         zmid, pmid, sza, vza, raa, wav, airc, gasc, temp, $
         dI0, dI1, dI2, TOMS_CLI, FILENAME = DIR_FITTING+'/Scattering_fit_'+TOMS[iozo]+'_'+CLDP[ipre]+'.dat'   
